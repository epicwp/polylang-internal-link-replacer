...
class HTML_Processor {
    ...
    public function find_internal_links( HtmlDomParser $dom, string $home_url ): array {
        ...
        foreach ( $dom_nodes as $dom_node ) {
            if ( ! $this->is_valid_internal_link( $dom_node ) ) {
                continue;
            }
    ...
    /**
     * Check if a DOM node is a valid internal link for processing.
     *
     * @param  mixed $dom_node DOM node to check.
     * @return bool True if the node is a valid internal link.
     */
    private function is_valid_internal_link( mixed $dom_node ): bool {
        if ( ! ( $dom_node instanceof SimpleHtmlDomInterface ) ) {
            return false;
        }

        // Skip a tags with hreflang or lang attribute a.k.a language switcher links.
        return ! $dom_node->hasAttribute( 'hreflang' ) && ! $dom_node->hasAttribute( 'lang' );
    }
